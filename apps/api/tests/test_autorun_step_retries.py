...
class _ApiErrorOnceLLM(LLMClient):
    def __init__(self, *, outputs: list[str]) -> None:
        req = httpx.Request("POST", "http://test/v1/chat/completions")
        self.queue: list[object] = [APIError(message="An error occurred during streaming", request=req, body=None), *outputs]
...
@pytest.fixture()
async def client_with_apierror_once_llm_and_embeddings(_ensure_test_database: None, test_database_url: str):
    outputs = [
        json.dumps(
            {
                "chapters": [
    ...
    llm = _ApiErrorOnceLLM(outputs=outputs)
    settings = Settings(
        database_url=test_database_url,
        cors_allow_origins="http://localhost:5173,http://127.0.0.1:5173",
    )
...
async def test_autorun_uses_latest_retry_settings_without_new_snapshot(client_with_failing_llm_and_embeddings):
    patched = await client_with_failing_llm_and_embeddings.patch(
        "/api/settings/output-spec",
        json={"auto_step_retries": 0, "auto_step_backoff_s": 0.0},
    )
    assert patched.status_code == 200
    assert patched.json()["auto_step_retries"] == 0
    assert patched.json()["auto_step_backoff_s"] == 0.0
    ...
    snap_id = snap.json()["id"]
    assert snap.json()["content"]["output_spec"]["auto_step_retries"] == 0
    ...
    patched2 = await client_with_failing_llm_and_embeddings.patch(
        "/api/settings/output-spec",
        json={"auto_step_retries": 2, "auto_step_backoff_s": 0.0},
    )
    assert patched2.status_code == 200
    assert patched2.json()["auto_step_retries"] == 2
    assert patched2.json()["auto_step_backoff_s"] == 0.0
...
async def test_autorun_retries_api_error_and_continues(client_with_apierror_once_llm_and_embeddings):
    await client_with_apierror_once_llm_and_embeddings.patch(
        "/api/settings/output-spec",
        json={"auto_step_retries": 2, "auto_step_backoff_s": 0.0},
    )

    brief = await client_with_apierror_once_llm_and_embeddings.post(
        "/api/briefs", json={"title": "测试作品", "content": {}}
    )
    brief_id = brief.json()["id"]
    snap = await client_with_apierror_once_llm_and_embeddings.post(
        f"/api/briefs/{brief_id}/snapshots", json={"label": "v1"}
    )
    snap_id = snap.json()["id"]

    run = await client_with_apierror_once_llm_and_embeddings.post(
        "/api/workflow-runs",
        json={"kind": "novel", "brief_snapshot_id": snap_id, "status": "queued", "state": {}},
    )
    run_id = run.json()["id"]

    started = await client_with_apierror_once_llm_and_embeddings.post(f"/api/workflow-runs/{run_id}/autorun/start")
    assert started.status_code == 200

    for _ in range(80):
        got = await client_with_apierror_once_llm_and_embeddings.get(f"/api/workflow-runs/{run_id}")
        if got.json()["status"] == "succeeded":
            break
        await asyncio.sleep(0.02)
    ...
    steps_resp = await client_with_apierror_once_llm_and_embeddings.get(f"/api/workflow-runs/{run_id}/steps")
    steps = steps_resp.json()
    outline_steps = [s for s in steps if s["step_name"] == "novel_outline"]
    assert len(outline_steps) >= 2
    assert any(s["status"] == "failed" for s in outline_steps)
